...
export default async function MessagesPage() {
  ...
  if (threadIds.length > 0) {
    ...
    // The other person in each direct/ride thread.
    ...
    // Meeting and horse context for ride/meeting_group threads.
      ...
      {threads.length > 0 ? (
        <div className="space-y-2">
          {threads.map((t) => {
            ...
            // Build thread title based on type.
            // meeting_group: "Ellerslie · 16 Jun" — no single "other" person.
            // ride: "Horse · Trainer name"
            // direct: other person's name
            const title =
              t.type === "ride"
                ? `${horse ? `${horse} · ` : "Ride · "}${other?.name ?? "Ride chat"}`
                : t.type === "meeting_group" && meeting
                ? `${meeting.track} · ${new Date(meeting.meeting_date + "T00:00:00").toLocaleDateString("en-NZ", { day: "numeric", month: "short" })}`
                : t.type === "meeting_group"
                ? "Meeting Group Chat"
                : other?.name ?? "Conversation";
            // Avatar: use track initial for group threads, other person's photo/name for the rest.
            const avatarName =
              t.type === "meeting_group"
                ? (meeting?.track ?? "Meeting")
                : other?.name;
            const avatarSrc =
              t.type === "meeting_group" ? undefined : other?.photo;
            return (
              <Link
                key={t.id}
